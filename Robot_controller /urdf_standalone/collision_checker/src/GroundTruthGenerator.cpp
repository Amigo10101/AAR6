  ...
  for (const auto &link_data : collision_data_) {
    ...
    for (const auto &geom : geometries) {
      ...
      if (fcl_geom) {
        ...
        auto *collision_obj = new fcl::CollisionObjectd(fcl_geom, transform);
...
bool GroundTruthGenerator::isSelfCollision(const Eigen::VectorXd &q) {
  ...
  if (fk_) {
    ...
    fk_->computeLinkTransforms(q, link_transforms);

    // Update FCL object transforms
